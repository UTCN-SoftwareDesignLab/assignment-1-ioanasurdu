...
public class RightsRolesRepositoryMySQL implements RightsRolesRepository {
    ...
    @Override
    public void createUserStatement(User user, List<Role> roles) {
        try {
            for (Role role : roles) {
                PreparedStatement createUserRoleStatement = connection
                        .prepareStatement("INSERT INTO `user_role` values (null, ?, ?)");
                createUserRoleStatement.setLong(1, user.getId());
                createUserRoleStatement.setLong(2, role.getId());
    ...
    @Override
    public void updateUserStatement(User user, List<Role> roles) {
        try {
            for (Role role : roles) {
                PreparedStatement updateUserStatement = connection
                        .prepareStatement("DELETE FROM user_role WHERE usename=?", Statement.RETURN_GENERATED_KEYS);
                updateUserStatement.setLong(1, user.getId());
    ...
    @Override
    public void deleteUserStatement(User user) {
        try {
